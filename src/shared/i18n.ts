...
const en: Record<string, string> = {
  ...
  'input.placeholder.connect': 'Go to “AI connections” above and open the AIs this mode needs',
  'input.stop': 'Stop',
  'input.send': 'Send',
  ...
  'error.input_not_found': 'Could not find the {provider} input box',
  'error.input_injection_failed': 'Could not type into {provider}: {detail}',
  'error.input_disappeared': 'The {provider} input box disappeared before the send was confirmed',
  'error.send_failed': 'Could not send to {provider}: {detail}',
  'error.send_rejected': '{provider} did not accept the message; the draft is still in the composer',
  'error.no_response_text': '{provider} produced no response text',
  'error.response_in_progress': '{provider} is already working on a response',
  'error.empty_message': 'The message is empty',
  'error.not_ready': '{provider} is not ready',
  'error.navigated_away': '{provider} navigated away during the workflow',
  ...
  'error.hackmd_no_link': 'The HackMD response is missing a publish link',
};
...
const zh: Record<string, string> = {
  ...
  'input.placeholder': '詢問已選取的 AI…', 'input.placeholder.processing': '流程執行中…', 'input.placeholder.connect': '請到上方「AI 連線」開啟此模式需要的 AI', 'input.stop': '停止', 'input.send': '送出',
  ...
  'error.input_injection_failed': '無法把文字輸入 {provider}：{detail}',
  'error.input_disappeared': '{provider} 的輸入框在確認送出前就消失了',
  'error.send_failed': '無法送出到 {provider}：{detail}',
  'error.send_rejected': '{provider} 沒有接受這則訊息，草稿還留在輸入框裡',
  'error.no_response_text': '{provider} 沒有產生任何回應文字',
  'error.response_in_progress': '{provider} 還有一則回應正在進行中',
  'error.empty_message': '訊息是空的',
  'error.not_ready': '{provider} 尚未就緒',
  'error.navigated_away': '{provider} 在流程進行中離開了頁面',
  ...
  'error.hackmd_failed': 'HackMD {status}：{detail}',
  'error.hackmd_no_link': 'HackMD 的回應沒有附上發佈連結',
};
...
const ja: Record<string, string> = {
  ...
  'input.placeholder': '選択したAIに質問…', 'input.placeholder.processing': '実行中…', 'input.placeholder.connect': '上部の「AI 接続」でこのモードに必要な AI を開いてください', 'input.stop': '停止', 'input.send': '送信',
...
const de: Record<string, string> = {
  ...
  'input.placeholder': 'Ausgewählte KIs fragen…', 'input.placeholder.processing': 'Workflow läuft…', 'input.placeholder.connect': 'Öffne die für diesen Modus benötigten KIs oben unter „KI-Verbindungen“', 'input.stop': 'Stopp', 'input.send': 'Senden',
...
const ko: Record<string, string> = {
  ...
  'input.placeholder': '선택한 AI에게 질문…', 'input.placeholder.processing': '워크플로 실행 중…', 'input.placeholder.connect': '위쪽 「AI 연결」에서 이 모드에 필요한 AI를 열어 주세요', 'input.stop': '중지', 'input.send': '전송',
