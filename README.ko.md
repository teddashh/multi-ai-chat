...
- 라이트/다크/시스템 표시 모드: 다크 모드는 기본적으로 OS를 따르며 설정에서 수동 전환 가능, 대비는 WCAG 기준 충족.
...
원탁 토론 중 provider가 실패하면 workflow가 일시 중지되고 **다시 시도**, **이번 발언 건너뛰기**, **취소** 중 하나를 선택할 수 있습니다. 건너뛰면 남은 원탁 토론 문맥에서 해당 응답을 안전한 자리 표시자로 대체하고 다음 AI로 계속하며, provider 오류 문구는 후속 발언에 전달되지 않습니다.
...
v0.2.x에 대한 많은 기여(전송/응답 안정성, 연결 복구, i18n 오류 처리, 다크 모드, 사이드 패널 UX, 아이콘 투명화, LICENSE 추가)에 특별히 감사드립니다 — [@DaveTseng2019](https://github.com/DaveTseng2019).
