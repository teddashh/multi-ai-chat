...
- ライト／ダーク／システムの表示モード。ダークは既定でOSに追従し、設定で手動切り替え可能。コントラストはWCAG準拠。
...
円卓討論中に provider が失敗するとワークフローが一時停止し、**再試行**、**この発言をスキップ**、または**中止**を選べます。スキップすると、残りの円卓コンテキストではその応答を安全なプレースホルダーに置き換えて次の AI へ進み、provider のエラー文は後続の発言に渡しません。
